// Fix Favorites Sync Script
// This script helps resolve favorites sync issues for existing users
// Run this in the browser console on any page of the app
...
const fixFavoritesSync = async () => {
  ...
  try {
    ...
    // Step 3: Get Firestore favorites
    ...
    // Step 4: Get recently viewed vendors
    ...
    if (duplicatePlaceIds.length > 0) {
      issues.push(`Vendors with duplicate placeIds: ${duplicatePlaceIds.map(([placeId]) => placeId).join(', ')}`);
    }
    ...
    // Step 9: Update localStorage with final state
...
console.log('✅ Favorites sync utilities loaded!');
