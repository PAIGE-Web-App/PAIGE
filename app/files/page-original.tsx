...
export default function FilesPage() {
  ...
  // Constants for subfolder limits
  ...
  // Calculate the current folder's level
  ...
  // Check if current folder has reached the subfolder level limit
  ...
  const handleAddSubfolder = async (name: string, description?: string, color?: string) => {
    ...
    // Check if adding a subfolder would exceed the level limit
                    ...
                    <Banner
                      message={
                        <>
                          ...
                          <button onClick={() => setShowUpgradeModal(true)} className="ml-2 font-semibold underline">Upgrade Plan</button> to create deeper folder structures.
                        </>
                      }
                      type="info"
                      onDismiss={() => setShowSubfolderLimitBanner(false)}
                    />
                  </div>
                )}
              </AnimatePresence>
