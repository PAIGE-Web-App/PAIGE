  ...
  try {
    ...
    const importPromises = incomingContacts.map(async (contact: any) => {
      ...
      try {
        ...
        let userFacingMessage = (errorResult as any).userFacingMessage || `Failed to import messages for ${contactName}. Please try again.`;
    ...
    const overallSuccess = importResults.every(result => (result as any).success);
    const totalImportedCount = importResults.reduce((sum, result) => sum + ((result as any).importedCount || 0), 0);

    if (!overallSuccess) {
      const failedContacts = importResults.filter(result => !(result as any).success);
      const errorMessage = `Failed to import messages for some contacts: ${failedContacts.map(f => (f as any).contactEmail).join(', ')}. Details: ${failedContacts.map(f => (f as any).message).join('; ')}`;
