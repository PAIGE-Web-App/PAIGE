...
export async function POST(req: NextRequest) {
  try {
    console.log('🚀 Gmail-send proxy API called');
    
    const requestBody = await req.json();
    console.log('📧 Proxying Gmail send request to Render service:', { 
      to: requestBody.to, 
    ...
    const renderResponse = await fetch(`${RENDER_SERVICE_URL}/gmail-send`, {
      method: 'POST',
      headers: {
    ...
    console.log('✅ Gmail send successful via Render service');
