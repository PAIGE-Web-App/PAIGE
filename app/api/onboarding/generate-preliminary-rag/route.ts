...
export async function POST(req: NextRequest) {
  ...
  const n8nUrl = process.env.N8N_WEBHOOK_URL || 'https://paigewedding.app.n8n.cloud/webhook-test/onboarding-rag';
    ...
    // Generate high-quality budget using our API (bypass n8n workflow completely)
    console.log('💰 Generating high-quality budget using our API...');
    
    try {
      const budgetResponse = await fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/generate-budget-rag`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'x-onboarding-free': 'true', // Flag to indicate this is from onboarding (free)
        },
        body: JSON.stringify({
          description: `Budget breakdown for a ${weddingData.weddingLocation || 'your location'} wedding for ${weddingData.guestCount || 120} guests`,
          totalBudget: weddingData.maxBudget || 50000,
          weddingDate: weddingDateString || new Date().toISOString(),
          budgetType: 'comprehensive',
          userId,
          userEmail: 'user@example.com',
          isOnboarding: true // Flag to indicate this is from onboarding (free)
        })
      });
      ...
      if (budgetResponse.ok) {
        ...
        if (budgetResult.success && budgetResult.budget) {
          ...
          console.log('✅ Generated high-quality budget using our API:', { budget: budget.total, categories: budget.categories?.length });
        } else {
          throw new Error('Budget API returned unsuccessful response');
        }
      } else {
        throw new Error(`Budget API returned ${budgetResponse.status}`);
      }
    } catch (budgetError) {
      console.log('⚠️ High-quality budget generation failed, using basic fallback:', budgetError);
      // Use basic fallback if budget API fails
      budget = {
    ...
    // Generate vendors using n8n workflow (keep this as is since vendor generation works well)
    if (!n8nResponse.ok) {
      console.log('⚠️ n8n webhook failed for vendors, using empty vendor data');
      vendors = {};
    } else {
      ...
      // Extract only vendors from RAG response (budget and todos generated by our APIs)
      // Handle both direct data and nested data formats
      const responseData = ragResult.data || ragResult;
      const { vendors: rawVendors } = responseData;
      
      // Clean the vendors data to remove undefined values
      ...
      // Clean only vendors data (budget and todos already generated by our high-quality APIs)
    ...
    try {
      ...
      // DO NOT save todos to Firestore during onboarding generation
      // Todo data will be saved only when user clicks "Complete Setup" on /onboarding/budget
      console.log('📋 Todo data prepared for onboarding (not saved to Firestore yet)');
      console.log(`📊 Generated ${todos.length} todos for onboarding`);

    // DO NOT save budget to Firestore during onboarding generation
    // Budget data will be saved only when user clicks "Complete Setup" on /onboarding/budget
    console.log('💰 Budget data prepared for onboarding (not saved to Firestore yet)');
