...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  try {
    ...
    if (err.code === "auth/email-already-in-use") {
      message = "That email is already in use. Please enter another email.";
...
const handleGoogleSignUp = async () => {
  ...
  try {
    ...
    // After successful Google sign-in, redirect to step 2 for onboarding details
    setStep(2);
  } catch (err: any) {
      ...
      <div className="w-full max-w-[1280px] flex">
        <div className="w-[40%] min-w-[400px] flex flex-col justify-center items-start px-12">
          ...
          <h4 className="text-[#364257] text-sm font-playfair font-normal mb-6 text-center w-full">
            ...
            type="submit"
            disabled={loading}
            className={`btn-primary w-full py-2 text-sm ${loading ? "bg-[#DCDCDC] cursor-not-allowed" : ""}`}
          >
          ...
          <button
            ...
            className="w-full py-2 text-sm border border-[#332B42] text-[#332B42] rounded-[5px] flex items-center justify-center gap-2"
          >
            ...
            <p className="text-xs text-center font-work-sans text-[#332B42] mt-8">
              ...
              <span className="text-[#A85C36] cursor-pointer font-medium underline hover:text-[#784528]">Login</span>
            </p>
...
{step === 3 && (
  <></>
        ...
        <div className="w-[60%] p-4 flex">
          <OnboardingVisual imageSrc="/rings.svg"/>
        </div>
      </div>
    </div>
  );
