...
interface EditVenueItemModalProps {
  isOpen: boolean;
  onClose: () => void;
  onUpdateVenueItem: (tableId: string, updates: Partial<TableType>) => void;
  venueItem: TableType;
}
...
export default function EditVenueItemModal({ isOpen, onClose, onUpdateVenueItem, venueItem }: EditVenueItemModalProps) {
  const [venueItemData, setVenueItemData] = useState({
    name: venueItem.name,
    type: venueItem.type as 'round' | 'long',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (venueItemData.name.trim()) {
      onUpdateVenueItem(venueItem.id, {
        ...
        >
          ...
          <div className="flex items-center justify-between p-4 md:p-6 border-b border-[#E0DBD7] flex-shrink-0">
            <div className="flex items-center space-x-3">
              ...
              <h5 className="h5 text-left text-lg md:text-xl">Edit Venue Item</h5>
            </div>
            <button
              onClick={handleClose}
          ...
          <div className="flex-1 overflow-y-auto p-4 md:p-6">
            ...
            <div className="mb-6">
              <p className="text-sm text-gray-600 text-left">
                Edit venue item details like name, shape, and description.
              </p>
            </div>
